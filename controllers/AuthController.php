...
class AuthController {
    ...
    public function login(): void {
        ...
        if ($this->getUsuarioModel()->isLocked($user, $lockoutTime)) {
            ...
            $detalhesLog = json_encode(['ocorrencia' => "Tentativa de login em conta bloqueada no aparelho: " . $dispositivo . " para o usuário: " . $login], JSON_UNESCAPED_UNICODE);
        ...
        if (\Security\PasswordService::verify($senha, $user['usu_senha_salt'] ?? '', $user['usu_senha_hash'])) {
            ...
            $detalhesLog = json_encode(['ocorrencia' => "Login realizado com sucesso no aparelho: " . $dispositivo], JSON_UNESCAPED_UNICODE);
            ...
            $detalhesLog = json_encode(['ocorrencia' => "Tentativa de login com senha incorreta no aparelho: " . $dispositivo . ". Tentativa {$attempts} de {$maxAttempts}."], JSON_UNESCAPED_UNICODE);
            Audit::log("LOGIN", "Usuarios", (int)$user['usu_id'], $detalhesLog);
